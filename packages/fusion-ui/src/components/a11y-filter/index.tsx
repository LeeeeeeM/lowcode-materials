...
export interface A11yFilterProps extends GridProps {
  /**
   * 是否展开，受控
   */
  expand?: boolean;
...
interface A11yTableInterface {
  onSearchCallback: (v: Record<string, any>) => void;
  onResetCallback: () => void;
}

const A11yFilter: React.FC<A11yFilterProps & A11yTableInterface> = ({
  expand: propsExpand,
  defaultExpand,
  hideExpandButton,
  buttons,
  ...
  const renderDefaultBtns = () => {
    const submitBtn = (
      <Form.Submit type="primary" validate onClick={(v: Record<string, any>) => {
        onSearch?.(v as any);
        onSearchCallback?.(v);
      }}>
    ...
    const resetBtn = <Form.Reset onClick={() => {
      onReset?.();
      onResetCallback?.();
    }}>{i18nBundle.secondaryBtnText}</Form.Reset>;
